...
"""
    collect(S::Subspace{n, d}) -> Vector{SymplecticVector{n,d}}

Return a vector containing all elements of the subspace generated by `S.basis`.
The subspace is finite since we are over a finite field.
"""
function collect(S::Subspace{n,d}) where {n,d}
    basis = S.basis
    r = length(basis)
    FF = finite_field(d)[1]
    ...
    if r == 0
        return [zero(SymplecticVector{n,d})]
    end

    # Get all elements of the field F_d
    field_elems = [FF(i) for i in 0:(d-1)]

    # Create all d^r tuples of coefficients
    combos = Base.Iterators.product((field_elems for _ in 1:r)...)

    # For each combination of coefficients, form the linear combination of the basis
    elements = SymplecticVector{n,d}[]
    for coeffs in combos
        v = zero(SymplecticVector{n,d})
...
"""
    collect(S::LagrangianSubspace{n, d}) -> Vector{SymplecticVector{n,d}}

Return a vector containing all elements of the Lagrangian subspace generated by `S.basis`.
"""
function collect(S::LagrangianSubspace{n,d}) where {n,d}
    return collect(Subspace{n,d}(S.basis; check=false))
end

"""
    collect(S::IsotropicSubspace{n, d}) -> Vector{SymplecticVector{n,d}}

Return a vector containing all elements of the isotropic subspace generated by `S.basis`.
"""
function collect(S::IsotropicSubspace{n,d}) where {n,d}
    # IsotropicSubspace is a Subspace with the additional isotropy property.
    # We can ignore that and just call collect on a Subspace constructed from the same basis.
    return collect(Subspace{n,d}(S.basis; check=false))
end

"""
    symplecticcomplement(W::Subspace{n,d}) -> Subspace{n,d}

Compute the symplectic complement of a subspace `W`.

`W` is a subspace of a symplectic vector space `V` of dimension 2n over `F_d`.
Its symplectic complement is defined by:
W^⊥ = { v ∈ V : w ⋆ v = 0 for all w ∈ W }.
This function returns a `Subspace{n,d}` whose basis spans `W^⊥`.
"""
function symplecticcomplement(W::Subspace{n,d}) where {n,d}
    FF = finite_field(d)[1]

    # Let r = dimension(W)
    r = length(W.basis)

    # If W is empty, the complement is the whole space
    if isempty(W.basis)
        # We can construct a standard basis for V = Z^n x X^n:
        basis = SymplecticVector{n,d}[]
        for i ∈ 1:n
            z = fill(FF(0), n); x = fill(FF(0), n)
            z[i] = FF(1)
            push!(basis, SymplecticVector{n,d}(z, x))
        end
        for i in 1:n
            z = fill(FF(0), n); x = fill(FF(0), n)
            x[i] = FF(1)
        ...
        return Subspace{n,d}(basis; check=false)
    end

    # Build the standard symplectic form matrix J:
    I_n = identity_matrix(FF, n)
    Z_n = zero_matrix(FF, n, n)
    J = [Z_n I_n;
        -I_n Z_n]

    # Represent W as a (2n × r) matrix by stacking w.z and w.x
    W_mat = hcat([vcat(w.z, w.x) for w in W.basis]...)

    # We want the null space of W_mat' * J, which is r×2n
    # M = W_mat' * J
    M = transpose(W_mat) * J

    # Compute RREF of M to find its null space
    # M * v = 0, we want solutions for v in (F_d)^{2n}
    rnk, R = rref(M)

    # Identify pivot columns
    pivots = Int[]
    for i ∈ 1:size(R,1)
        pivot_col = findfirst(!iszero, R[i,:])
        if pivot_col !== nothing
            push!(pivots, pivot_col)
        end
    end

    free_cols = setdiff([1:2n...], pivots)
    complement_basis = SymplecticVector{n,d}[]

    # For each free variable, set it = 1 and others = 0 and solve for pivot vars
    for fc in free_cols
        v_sol = fill(FF(0), 2n)
        v_sol[fc] = FF(1)

        # Solve for pivot variables from the RREF
        # R * v = 0 ⇒ For each pivot row:
        for i in 1:size(R,1)
            pivot_col = findfirst(!iszero, R[i,:])
            if pivot_col != nothing
                # The pivot variable is at R[i, pivot_col].
                # Equation: R[i,:]·v = 0
                # v[pivot_col] = - (sum_{j>pivot_col} R[i,j]*v[j]) / R[i,pivot_col]
                sum_rest = FF(0)
                for j in (pivot_col+1):2n
                    sum_rest += R[i,j]*v_sol[j]
                end
                # pivot element should be invertible, typically R[i,pivot_col] = 1 in RREF
                # but we will write the general form:
                v_sol[pivot_col] = -sum_rest * inv(R[i,pivot_col])
            end
        end

        # Now v_sol is a solution in F_d^{2n}. Construct a SymplecticVector from it.
        z_part = v_sol[1:n]
        x_part = v_sol[n+1:2n]
        push!(complement_basis, SymplecticVector{n,d}(z_part, x_part))
    end

    return Subspace{n,d}(complement_basis; check=true)
end

"""
    symplectic_complement(W::IsotropicSubspace{n,d}) -> Subspace{n,d}

For an isotropic subspace, the same method as above.
We just dispatch to `symplectic_complement(W::Subspace{n,d})`.
"""
function symplecticcomplement(W::IsotropicSubspace{n,d}) where {n,d}
    return symplecticcomplement(Subspace{n,d}(W.basis; check=false))
end

"""
    symplectic_complement(W::LagrangianSubspace{n,d}) -> Subspace{n,d}

For a Lagrangian subspace (dim=n), we know W^⊥=W.
Just return W (wrapped in a Subspace).
"""
function symplecticcomplement(W::LagrangianSubspace{n,d}) where {n,d}
    # If W is truly Lagrangian, dimension(W)=n, so W^⊥=W
    return Subspace{n,d}(W.basis; check=false)
end
